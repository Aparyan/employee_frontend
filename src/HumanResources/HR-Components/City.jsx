...
const State = () => {
  ...
  const handleAddSave = async ()=>{
    const response = await fetch("http://localhost:5000/api/hr/city/",{
      method: "POST",
  ...
  const handleDelete = async (id) =>{
    const response = await fetch("http://localhost:5000/api/hr/city/"+id,{
      method:"DELETE"
    })
  ...
  const handleEdit = (id,index) =>{
    ...
    setCity(tableData[index].city)
    setEditId(id)
    setShowEdit(true)
  }

  const handleEditSave = async ()=>{
    const response = await fetch("http://localhost:5000/api/hr/city/"+editId,{
      method: "PATCH",
      body: JSON.stringify({country , state, city}),
      headers: {
        "content-type": "application/json"
  ...
  return (
    <div className="table-container">
      <div className="header">
        ...
        <h3 className="text">City Details</h3>
