...
const Country = () => {
  ...
  const handleAddSave = async ()=>{
    const response = await fetch("http://localhost:5000/api/hr/country/",{
      method: "POST",
  ...
  const handleDelete = async (id) =>{
    const response = await fetch("http://localhost:5000/api/hr/country/"+id,{
      method:"DELETE"
    })
  ...
  const handleEditSave = async ()=>{
    const response = await fetch("http://localhost:5000/api/hr/country/"+editId,{
      method: "PATCH",
      body: JSON.stringify({country}),
      headers: {
        "content-type": "application/json"
